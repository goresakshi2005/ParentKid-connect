 ...
 */
const FeatureGuard = ({ feature, children, showOverlay = true }) => {
  const { user } = useAuth();
  ...
  return (
    <div className="relative overflow-hidden w-full h-full">
      ...
      <motion.div 
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-black/40 backdrop-blur-[2px] rounded-xl p-6 text-center"
      >
        <div className="bg-white/90 dark:bg-gray-800/90 p-8 rounded-2xl shadow-2xl max-w-md border border-white/20">
          ...
          <button
            ...
            className="w-full py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-bold hover:shadow-lg transition-all transform hover:scale-[1.02]"
          >
            Upgrade Now
          </button>
        </div>
